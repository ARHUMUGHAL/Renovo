...
const Navbar = () => {
  ...
  const ToggleMenu = () => {
      ...
      if (toggleMenu) {
        if (isMenuOpen) {
          gsap.to(toggleMenu, { x: '120%', opacity: 0, duration: 1, onComplete: () => toggleMenu.style.display = 'none' });
        }
        else {
          toggleMenu.style.display = 'inline-block';
          gsap.fromTo(toggleMenu, { x: '120%', opacity: 0 }, { x: '100%', opacity: 1, duration: 1 });
        }
  ...
  return (
    <div className='navbar fixed top-0 left-0 z-[999] w-full px-[20px]  flex justify-between items-start text-[36px] h-screen text-white'>
      <div className={navbar ? 'w-full h-[100px] z-[999] flex justify-between items-center bg-black-500 backdrop-blur-[8px] transition duration-[.5s] linear' : 'w-full h-[100px] z-[999] flex justify-between items-center transition duration-[.5s] linear'}>
        <div className="nav-logo font-playfair">Renovo</div>
        <div className='cursor-pointer md:hidden' onClick={ToggleMenu}><MdOutlineSegment /></div>
